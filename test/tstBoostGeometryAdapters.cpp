...
BOOST_AUTO_TEST_CASE( equals )
{
    ...
    BOOST_TEST(
        bg::equals( box, details::Box{{{1., 2., 3.}}, {{4., 5., 6.}}} ) );
    BOOST_TEST(
        !bg::equals( box, details::Box{{{0., 0., 0.}}, {{1., 1., 1.}}} ) );
    BOOST_TEST(
    ...
    // of ArborX points.
    BOOST_TEST( bg::equals( box, bg::model::box<details::Point>(
                                     {{1., 2., 3.}}, {{4., 5., 6.}} ) ) );
    BOOST_TEST( !bg::equals( box, bg::model::box<details::Point>(
                                      {{0., 0., 0.}}, {{0., 0., 0.}} ) ) );
}
...
BOOST_AUTO_TEST_CASE( centroid )
{
    ...
    BOOST_TEST( equals( bg::return_centroid<details::Point>( empty_box ),
                        {{0., 0., 0.}} ) );
    BOOST_TEST(
        equals( details::returnCentroid( empty_box ), {{0., 0., 0.}} ) );

    details::Box unit_box = {{{0., 0., 0.}}, {{1., 1., 1.}}};
    BOOST_TEST( equals( bg::return_centroid<details::Point>( unit_box ),
                        {{.5, .5, .5}} ) );
    BOOST_TEST( equals( details::returnCentroid( unit_box ), {{.5, .5, .5}} ) );

    details::Point a_point = {{1., 2., 3.}};
    BOOST_TEST(
        equals( bg::return_centroid<details::Point>( a_point ), a_point ) );
