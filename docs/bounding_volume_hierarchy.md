...
: checks whether the tree has no leaf.
...
: finds all leaves that statisfy given predicates, e.g. nearest to a point or intersecting with a box or a sphere.
...
#### Type requirements
A specialization of [`ArborX::Traits::Access`](https://github.com/dalg24/ArborX/blob/docs/docs/traits.md#arborxtraitsaccess) must match `Primitives` as the first template argument and `PrimitivesTag` as second argument.  
...
```
Checks if the tree has no leaf.
## Parameters
(none)
## Return value
...
```
Returns the smalest axis-aligned box able to contain all leaves stored in the tree or an invalid box if the tree is empty.
## Parameters
(none)
## Return value
...
#### Type requirements
A specialization of [`ArborX::Traits::Access`](https://github.com/dalg24/ArborX/blob/docs/docs/traits.md#arborxtraitsaccess) must match the `Predicates` as first template argument and `PredicatesTag` as second argument.  
...
* [`ArborX::nearest()`](https://github.com/dalg24/ArborX/blob/docs/docs/predicates.md#arborxnearest)
* [`ArborX::intersects()`](https://github.com/dalg24/ArborX/blob/docs/docs/predicates.md#arborxintersects)

`indices`
: position of the primitives that satisfy the predicates.  
`offsets`
: predicate offsets for one-dimensional storage.
