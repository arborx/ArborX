/****************************************************************************
 * Copyright (c) 2017-2022 by the ArborX authors                            *
 * All rights reserved.                                                     *
 *                                                                          *
 * This file is part of the ArborX library. ArborX is                       *
 * distributed under a BSD 3-clause license. For the licensing terms see    *
 * the LICENSE file in the top-level directory.                             *
 *                                                                          *
 * SPDX-License-Identifier: BSD-3-Clause                                    *
 ****************************************************************************/
#ifndef ARBORX_DETAILS_ALGORITHMS_HPP
#define ARBORX_DETAILS_ALGORITHMS_HPP

#include <ArborX_Box.hpp>
#include <ArborX_DetailsKokkosExtArithmeticTraits.hpp>
#include <ArborX_DetailsKokkosExtMathFunctions.hpp> // isfinite
#include <ArborX_Point.hpp>
#include <ArborX_Sphere.hpp>

#include <Kokkos_Macros.hpp>

namespace ArborX
{
namespace Details
{

template <int DIM>
KOKKOS_INLINE_FUNCTION constexpr bool equals(PointD<DIM> const &l,
                                             PointD<DIM> const &r)
{
  for (int d = 0; d < DIM; ++d)
    if (l[d] != r[d])
      return false;
  return true;
}

template <int DIM>
KOKKOS_INLINE_FUNCTION constexpr bool equals(BoxD<DIM> const &l,
                                             BoxD<DIM> const &r)
{
  return equals(l.minCorner(), r.minCorner()) &&
         equals(l.maxCorner(), r.maxCorner());
}

template <int DIM>
KOKKOS_INLINE_FUNCTION constexpr bool equals(SphereD<DIM> const &l,
                                             SphereD<DIM> const &r)
{
  return equals(l.centroid(), r.centroid()) && l.radius() == r.radius();
}

template <int DIM>
KOKKOS_INLINE_FUNCTION bool isValid(PointD<DIM> const &p)
{
  using KokkosExt::isfinite;
  for (int d = 0; d < DIM; ++d)
    if (!isfinite(p[d]))
      return false;
  return true;
}

template <int DIM>
KOKKOS_INLINE_FUNCTION bool isValid(BoxD<DIM> const &b)
{
  using KokkosExt::isfinite;
  for (int d = 0; d < DIM; ++d)
  {
    auto const r_d = b.maxCorner()[d] - b.minCorner()[d];
    if (r_d <= 0 || !isfinite(r_d))
      return false;
  }
  return true;
}

template <int DIM>
KOKKOS_INLINE_FUNCTION bool isValid(SphereD<DIM> const &s)
{
  using KokkosExt::isfinite;
  return isValid(s.centroid()) && isfinite(s.radius()) && (s.radius() >= 0.);
}

// distance point-point
template <int DIM>
KOKKOS_INLINE_FUNCTION float distance(PointD<DIM> const &a,
                                      PointD<DIM> const &b)
{
  float distance_squared = 0.0;
  for (int d = 0; d < DIM; ++d)
  {
    float tmp = b[d] - a[d];
    distance_squared += tmp * tmp;
  }
  return std::sqrt(distance_squared);
}

// distance point-box
template <int DIM>
KOKKOS_INLINE_FUNCTION float distance(PointD<DIM> const &point,
                                      BoxD<DIM> const &box)
{
  PointD<DIM> projected_point;
  for (int d = 0; d < DIM; ++d)
  {
    if (point[d] < box.minCorner()[d])
      projected_point[d] = box.minCorner()[d];
    else if (point[d] > box.maxCorner()[d])
      projected_point[d] = box.maxCorner()[d];
    else
      projected_point[d] = point[d];
  }
  return distance(point, projected_point);
}

// distance point-sphere
template <int DIM>
KOKKOS_INLINE_FUNCTION float distance(PointD<DIM> const &point,
                                      SphereD<DIM> const &sphere)
{
  using KokkosExt::max;
  return max(distance(point, sphere.centroid()) - sphere.radius(), 0.f);
}

// distance box-box
template <int DIM>
KOKKOS_INLINE_FUNCTION float distance(BoxD<DIM> const &box_a,
                                      BoxD<DIM> const &box_b)
{
  float distance_squared = 0.;
  for (int d = 0; d < DIM; ++d)
  {
    auto const a_min = box_a.minCorner()[d];
    auto const a_max = box_a.maxCorner()[d];
    auto const b_min = box_b.minCorner()[d];
    auto const b_max = box_b.maxCorner()[d];
    if (a_min > b_max)
    {
      float const delta = a_min - b_max;
      distance_squared += delta * delta;
    }
    else if (b_min > a_max)
    {
      float const delta = b_min - a_max;
      distance_squared += delta * delta;
    }
    else
    {
      // The boxes overlap on this axis: distance along this axis is zero.
    }
  }
  return std::sqrt(distance_squared);
}

// distance box-sphere
template <int DIM>
KOKKOS_INLINE_FUNCTION float distance(SphereD<DIM> const &sphere,
                                      BoxD<DIM> const &box)
{
  using KokkosExt::max;

  float distance_center_box = distance(sphere.centroid(), box);
  return max(distance_center_box - sphere.radius(), 0.f);
}

// expand an axis-aligned bounding box to include a point
template <int DIM>
KOKKOS_INLINE_FUNCTION void expand(BoxD<DIM> &box, PointD<DIM> const &point)
{
  box += point;
}

// expand an axis-aligned bounding box to include another box
template <int DIM>
KOKKOS_INLINE_FUNCTION void expand(BoxD<DIM> &box, BoxD<DIM> const &other)
{
  box += other;
}

// Needed to be able to use expand(box, box) in a Kokkos::parallel_reduce() in
// which case the arguments must be declared volatile.
template <int DIM>
KOKKOS_INLINE_FUNCTION void expand(BoxD<DIM> volatile &box,
                                   BoxD<DIM> const volatile &other)
{
  box += other;
}

// expand an axis-aligned bounding box to include a sphere
template <int DIM>
KOKKOS_INLINE_FUNCTION void expand(BoxD<DIM> &box, SphereD<DIM> const &sphere)
{
  using KokkosExt::max;
  using KokkosExt::min;
  for (int d = 0; d < DIM; ++d)
  {
    box.minCorner()[d] =
        min(box.minCorner()[d], sphere.centroid()[d] - sphere.radius());
    box.maxCorner()[d] =
        max(box.maxCorner()[d], sphere.centroid()[d] + sphere.radius());
  }
}

// check if two axis-aligned bounding boxes intersect
template <int DIM>
KOKKOS_INLINE_FUNCTION constexpr bool intersects(BoxD<DIM> const &box,
                                                 BoxD<DIM> const &other)
{
  for (int d = 0; d < DIM; ++d)
    if (box.minCorner()[d] > other.maxCorner()[d] ||
        box.maxCorner()[d] < other.minCorner()[d])
      return false;
  return true;
}

template <int DIM>
KOKKOS_INLINE_FUNCTION constexpr bool intersects(PointD<DIM> const &point,
                                                 BoxD<DIM> const &other)
{
  for (int d = 0; d < DIM; ++d)
    if (point[d] > other.maxCorner()[d] || point[d] < other.minCorner()[d])
      return false;
  return true;
}

// check if a sphere intersects with an  axis-aligned bounding box
template <int DIM>
KOKKOS_INLINE_FUNCTION bool intersects(SphereD<DIM> const &sphere,
                                       BoxD<DIM> const &box)
{
  return distance(sphere.centroid(), box) <= sphere.radius();
}

template <int DIM>
KOKKOS_INLINE_FUNCTION bool intersects(SphereD<DIM> const &sphere,
                                       PointD<DIM> const &point)
{
  return distance(sphere.centroid(), point) <= sphere.radius();
}

template <int DIM>
KOKKOS_INLINE_FUNCTION bool intersects(PointD<DIM> const &point,
                                       SphereD<DIM> const &sphere)
{
  return intersects(sphere, point);
}

// calculate the centroid of a box
template <int DIM>
KOKKOS_INLINE_FUNCTION void centroid(BoxD<DIM> const &box, PointD<DIM> &c)
{
  for (int d = 0; d < DIM; ++d)
    c[d] = (box.minCorner()[d] + box.maxCorner()[d]) / 2;
}

template <int DIM>
KOKKOS_INLINE_FUNCTION void centroid(PointD<DIM> const &point, PointD<DIM> &c)
{
  c = point;
}

template <int DIM>
KOKKOS_INLINE_FUNCTION void centroid(SphereD<DIM> const &sphere, PointD<DIM> &c)
{
  c = sphere.centroid();
}

template <int DIM>
KOKKOS_INLINE_FUNCTION PointD<DIM> returnCentroid(PointD<DIM> const &point)
{
  return point;
}

template <int DIM>
KOKKOS_INLINE_FUNCTION PointD<DIM> returnCentroid(BoxD<DIM> const &box)
{
  PointD<DIM> c;
  for (int d = 0; d < DIM; ++d)
    c[d] = (box.minCorner()[d] + box.maxCorner()[d]) / 2;
  return c;
}

template <int DIM>
KOKKOS_INLINE_FUNCTION PointD<DIM> returnCentroid(SphereD<DIM> const &sphere)
{
  return sphere.centroid();
}

// transformation that maps the unit cube into a new axis-aligned box
// NOTE safe to perform in-place
template <int DIM>
KOKKOS_INLINE_FUNCTION void
translateAndScale(PointD<DIM> const &in, PointD<DIM> &out, BoxD<DIM> const &ref)
{
  for (int d = 0; d < DIM; ++d)
  {
    auto const a = ref.minCorner()[d];
    auto const b = ref.maxCorner()[d];
    out[d] = (a != b ? (in[d] - a) / (b - a) : 0);
  }
}

} // namespace Details
} // namespace ArborX

#endif
